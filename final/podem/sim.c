...
sim2()
{
    ...
    for (i = 0; i < ncktin; i++) {
      ...
      /* if a input has changed, schedule the gates connected to it */
        if (sort_wlist[i]->flag2 & CHANGED) {
            sort_wlist[i]->flag2 &= ~CHANGED;
            for (j = 0; j < sort_wlist[i]->nout; j++) {
                if (sort_wlist[i]->onode[j]->nout) {
                    sort_wlist[i]->onode[j]->owire[0]->flag2 |= SCHEDULED;
                }
            }
        }
    } // for every input
     ...
     * it is correct to evaluate the wires in increasing order.   */
    for (i = ncktin; i < ncktwire; i++) {
        if (sort_wlist[i]->flag2 & SCHEDULED) {
            sort_wlist[i]->flag2 &= ~SCHEDULED;
            evaluate2(sort_wlist[i]->inode[0]);
            if (sort_wlist[i]->flag2 & CHANGED) {
                sort_wlist[i]->flag2 &= ~CHANGED;
                for (j = 0; j < sort_wlist[i]->nout; j++) {
                    if (sort_wlist[i]->onode[j]->nout) {
...
nptr n;
{
    ...
    switch(n->type) {
        ...
        case NAND:
            new_value = 1;
            for (i = 0; i < n->nin; i++) {
                new_value = ANDTABLE[n->iwire[i]->value2][new_value];
            }
            if (n->type == NAND) {
                new_value = INV[new_value];
            }
        ...
        case NOR:
            new_value = 0;
            for (i = 0; i < n->nin; i++) {
                new_value = ORTABLE[n->iwire[i]->value2][new_value];
            }
            if (n->type == NOR) {
                new_value = INV[new_value];
            }
            break;
        case NOT:
            new_value = INV[n->iwire[0]->value2];
            break;
        case XOR:
            new_value = XORTABLE[n->iwire[0]->value2][n->iwire[1]->value2];
            break;
        case EQV:
            new_value =INV[(XORTABLE[n->iwire[0]->value2][n->iwire[1]->value2])];
            break;
    }
    if (old_value != new_value) {
        n->owire[0]->flag2 |= CHANGED;
        n->owire[0]->value2 = new_value;
    }
    return;
}/* end of evaluate */
